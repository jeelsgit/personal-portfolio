      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {PROJECT_DATA.map((project, index) => (
            <article 
              key={index} 
              className="group bg-slate-850 rounded-lg border border-slate-800 hover:border-emerald-500/30 transition-all duration-300 hover:-translate-y-2 flex flex-col h-full overflow-hidden"
            >
              {project.imageUrl && (
                <div className="relative aspect-[16/10] overflow-hidden bg-slate-900 border-b border-slate-800">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-850 via-transparent to-transparent pointer-events-none"></div>
                </div>
              )}
